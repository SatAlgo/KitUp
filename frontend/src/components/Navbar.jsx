...
function Navbar() {
  // 1. FLASH-PROOF INITIALIZATION
  const [user, setUser] = useState(() => {
    try {
      const data = localStorage.getItem("KitUp_User");
      // Check for 'undefined' string which often causes the flicker
  ...
  // 4. INITIALS LOGIC (Handles Atlas 'fullname' or 'name' fields)
  const getInitials = (user) => {
    if (!user) return "U";
    const displayName = user.fullname || user.name || user.firstName || user.email || "User";
  ...
  const navItems = (
    <>
      <li><NavLink to="/" className={({ isActive }) => isActive ? "underline text-lime-400 font-bold" : "hover:text-lime-400"}>Home</NavLink></li>
      <li><NavLink to="/Essentials" className={({ isActive }) => isActive ? "underline text-lime-400 font-bold" : "hover:text-lime-400"}>Essentials</NavLink></li>
    </>
  );

  return (
    <div className="max-w-screen-2xl container mx-auto md:px-6 px-4 bg-white dark:bg-slate-900 dark:text-white fixed top-0 left-0 right-0 z-50 shadow-sm border-b dark:border-slate-800">
      <div className="navbar py-4">
        <div className="navbar-start">
          <div className="dropdown">
            ...
            <ul tabIndex={0} className="menu menu-md dropdown-content bg-white dark:bg-slate-800 rounded-box z-[1] mt-3 w-52 p-2 shadow-xl">{navItems}</ul>
          </div>
          <Link to="/" className="flex items-center gap-2">
            <img src={pic} className="h-10 w-10 rounded-full" alt="Logo" />
            <span className="text-lime-500 text-3xl font-black italic tracking-tighter">KitUp</span>
          </Link>
        </div>

        <div className="navbar-end space-x-4">
          <div className="hidden lg:flex"><ul className="menu menu-horizontal px-1 font-semibold">{navItems}</ul></div>
          
          <button onClick={() => setTheme(theme === "dark" ? "light" : "dark")} className="p-2 rounded-full hover:bg-slate-100 dark:hover:bg-slate-800 transition-all text-xl">
            {theme === "dark" ? "☀️" : "🌙"}
          </button>
          ...
          <div className="relative">
            {!user ? (
              <>
                <button 
                  className="bg-black text-white dark:bg-white dark:text-black px-6 py-2 rounded-full font-bold hover:opacity-80 transition-all" 
              ...
              <div className="dropdown dropdown-end">
                <label tabIndex={0} className="cursor-pointer">
                  <div className="w-10 h-10 rounded-full bg-lime-400 text-black flex items-center justify-center font-black border-2 border-white shadow-md hover:scale-105 transition-transform">
                    {getInitials(user)}
                  </div>
                </label>
                <ul tabIndex={0} className="dropdown-content menu p-2 shadow-2xl bg-white dark:bg-slate-800 rounded-box w-52 mt-4 border dark:border-slate-700">
                  <li className="px-4 py-2 font-bold border-b dark:border-slate-700 mb-2 text-slate-700 dark:text-slate-200">
                    {user.fullname || user.name || "User"}
                  </li>
                  <li><Link to="/profile">My Profile</Link></li>
                  <li>
                    <button className="text-red-500 font-bold" onClick={() => { localStorage.clear(); window.location.href = "/"; }}>
                      Logout
                    </button>
                  </li>
                </ul>
