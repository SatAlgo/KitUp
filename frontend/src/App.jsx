...
function App() {
  ...
  useEffect(() => {
    const checkInactivity = () => {
      // CHANGED: Use localStorage to match your Navbar/Login logic
      ...
      try {
        ...
        // Check if lastActive exists, if not, initialize it
        if (!user.lastActive) {
            user.lastActive = currentTime;
            localStorage.setItem("KitUp_User", JSON.stringify(user));
            return;
        }
    ...
    const updateActivity = () => {
      ...
      if (data && data !== "undefined") {
        try {
          ...
          // Silent catch for malformed data
        }
      }
    };
    ...
    // 2. Event listeners to detect activity
